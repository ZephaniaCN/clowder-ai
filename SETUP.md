...
This auto-starts Redis, builds packages, and launches both API (port 3002) and Frontend (port 3001).
...
|---------|------|----------|
| Frontend (Next.js) | 3001 | Yes |
| API Backend | 3002 | Yes |
...
- Make sure `NEXT_PUBLIC_API_URL=http://localhost:3002` is set
...
自动启动 Redis，构建包，启动 API（端口 3002）和前端（端口 3001）。
...
**前端连不上 API？**
- 确认设了 `NEXT_PUBLIC_API_URL=http://localhost:3002`
- API 必须在前端加载前启动
