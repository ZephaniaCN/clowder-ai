...
describe('BacklogStore assignProjectId', () => {
  /** @type {import('../dist/domains/cats/services/stores/ports/BacklogStore.js').BacklogStore} */
  let BacklogStore;

  beforeEach(async () => {
    const mod = await import('../dist/domains/cats/services/stores/ports/BacklogStore.js');
    BacklogStore = mod.BacklogStore;
  });

  test('assigns projectId to orphan item', () => {
    const store = new BacklogStore();
    const item = store.create({
      userId: 'u1',
      title: 'orphan',
    ...
    const updated = store.assignProjectId(item.id, 'proj-1');
    assert.equal(updated.projectId, 'proj-1');
    assert.ok(updated.updatedAt >= item.updatedAt);
    assert.ok(updated.audit.some((a) => a.detail.includes('backfill projectId: proj-1')));

    const fetched = store.get(item.id);
    assert.equal(fetched.projectId, 'proj-1');
  });

  test('is idempotent when projectId already matches', () => {
    const store = new BacklogStore();
    const item = store.create({
      userId: 'u1',
      title: 't',
    ...
    assert.equal(updated.audit.length, item.audit.length);
  });

  test('returns null for missing item', () => {
    const store = new BacklogStore();
    assert.equal(store.assignProjectId('nope', 'proj-1'), null);
  });
});
